...
// Global rule: ensure only one video plays at a time
    ...
    if (APP_DATA) {
        ...
        if (hasOldTournamentData || hasOldTeamData || hasOldSquadsData || hasOldBranding || hasAllStarsOctavos) {
            APP_DATA = INITIAL_DATA;
            localStorage.setItem('amanecer_data', JSON.stringify(APP_DATA));
            localStorage.setItem('amanecer_data_v', '3.0'); // Bump version
        }
    }
    ...
    document.getElementById('login-form').onsubmit = (e) => {
        ...
        const mlbbId = document.getElementById('login-mlbb-id').value;
        const mlbbZone = document.getElementById('login-mlbb-zone').value;
    ...
    document.getElementById('register-form').onsubmit = (e) => {
        ...
        const mlbbId = document.getElementById('register-mlbb-id').value;
        const mlbbZone = document.getElementById('register-mlbb-zone').value;
        ...
        alert("¡Correo enviado exitosamente!\n\nSimulador de Bandeja de Entrada:\n\nDe: soporte@amaneceresports.com\nAsunto: Código de Verificación Amanecer\n\nHola " + tempRegistrationUser.username + ",\nTu código de verificación es: " + expectedVerificationCode + "\n\nIncorpóralo en la ventana emergente para activar tu cuenta.");
    ...
    function showUserDashboard() {
        ...
        const mlbbSection = currentUser.mlbbId ? `
            <div class="mlbb-stats-dashboard mt-30 glass" style="padding: 30px; border-radius: 20px; border: 1px solid ${currentUser.verified ? 'var(--accent-orange)' : 'rgba(255,255,255,0.1)'};">
                <div class="stats-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; flex-wrap: wrap; gap: 15px;">
                    <div>
                        <h3 style="color: var(--accent-orange); margin-bottom: 5px;">
                            <i class="fas fa-id-card"></i> Datos de Jugador MLBB
                        </h3>
                        <p style="font-size: 0.8rem; opacity: 0.7;">ID: ${currentUser.mlbbId} (${currentUser.mlbbZone || '????'}) | Region: LATAM SUR</p>
                    </div>
                    <div style="background: ${currentUser.verified ? 'rgba(255, 140, 0, 0.2)' : 'rgba(255,255,255,0.05)'}; padding: 5px 15px; border-radius: 50px; font-size: 0.75rem; border: 1px solid ${currentUser.verified ? 'var(--accent-orange)' : 'rgba(255,255,255,0.2)'}; color: ${currentUser.verified ? 'var(--accent-orange)' : '#fff'};">
                        <i class="fas ${currentUser.verified ? 'fa-check-circle' : 'fa-clock'}"></i> ${currentUser.verified ? 'PERFIL VERIFICADO' : 'PENDIENTE DE VERIFICACIÓN'}
                    </div>
                </div>
                
                <div class="stats-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)); gap: 15px;">
                    <div class="stat-card glass" style="padding: 15px; text-align: center; background: rgba(0,0,0,0.2);">
                        <span class="val" style="display: block; font-size: 1.2rem; font-weight: 900; color: #fff;">${stats.rank}</span>
                        <span class="lbl" style="font-size: 0.6rem; opacity: 0.5; text-transform: uppercase;">Rango</span>
                    </div>
                    <div class="stat-card glass" style="padding: 15px; text-align: center; background: rgba(0,0,0,0.2);">
                        <span class="val" style="display: block; font-size: 1.2rem; font-weight: 900; color: #fff;">${stats.matches}</span>
                        <span class="lbl" style="font-size: 0.6rem; opacity: 0.5; text-transform: uppercase;">Partidas</span>
                    </div>
                    <div class="stat-card glass" style="padding: 15px; text-align: center; background: rgba(0,0,0,0.2);">
                        <span class="val" style="display: block; font-size: 1.2rem; font-weight: 900; color: var(--accent-orange);">${stats.winrate}</span>
                        <span class="lbl" style="font-size: 0.6rem; opacity: 0.5; text-transform: uppercase;">Win Rate</span>
                    </div>
                    <div class="stat-card glass" style="padding: 15px; text-align: center; background: rgba(0,0,0,0.2);">
                        <span class="val" style="display: block; font-size: 1.2rem; font-weight: 900; color: #fff;">${stats.mvps}</span>
                        <span class="lbl" style="font-size: 0.6rem; opacity: 0.5; text-transform: uppercase;">MVPs</span>
                    </div>
                </div>

                ${!currentUser.verified ? `
                    <div class="update-stats-form mt-30 p-20 glass" style="border: 1px solid rgba(255,140,0,0.2); background: rgba(255,140,0,0.02);">
                        <h4 style="font-size: 0.8rem; margin-bottom: 15px; color: var(--accent-orange);">ACTUALIZAR ESTADÍSTICAS VERÍDICAS</h4>
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(100px, 1fr)); gap: 10px;">
                            <input type="text" id="edit-rank" placeholder="Rango (Ej: Mitico 25*)" class="glass" style="padding:8px; font-size:0.8rem;" value="${stats.rank !== 'UNRANKED' ? stats.rank : ''}">
                            <input type="number" id="edit-matches" placeholder="Total Partidas" class="glass" style="padding:8px; font-size:0.8rem;" value="${stats.matches || ''}">
                            <input type="text" id="edit-wr" placeholder="Win Rate %" class="glass" style="padding:8px; font-size:0.8rem;" value="${stats.winrate !== '0%' ? stats.winrate : ''}">
                            <input type="number" id="edit-mvps" placeholder="Total MVPs" class="glass" style="padding:8px; font-size:0.8rem;" value="${stats.mvps || ''}">
                        </div>
                        <div class="proof-upload mt-15 p-20" style="background: rgba(255,255,255,0.02); border: 2px dashed rgba(255,255,255,0.1); border-radius: 12px; text-align: center; cursor: pointer;" onclick="document.getElementById('proof-file').click()">
                            <i class="fas fa-camera fa-2x mb-10" style="opacity:0.3;"></i>
                            <p style="font-size:0.75rem; opacity:0.6;">Sube una captura de tu perfil oficial (Screenshot)</p>
                            <input type="file" id="proof-file" style="display:none;" onchange="this.previousElementSibling.previousElementSibling.classList.replace('fa-camera','fa-file-image'); this.previousElementSibling.innerHTML='Archivo cargado: ' + this.files[0].name">
                        </div>
                        <button class="btn btn-primary btn-sm mt-15 w-100" onclick="updatePlayerStats()">ENVIAR A REVISIÓN OFICIAL</button>
                    </div>
                ` : ''}
            </div>
        ` : `
            <div class="mlbb-sync-prompt mt-30 glass" style="padding: 40px; text-align: center; border: 1px dashed rgba(255,255,255,0.2);">
                <i class="fas fa-link fa-2x mb-20" style="opacity: 0.3;"></i>
                <h4>No hay un ID de MLBB vinculado</h4>
                <p style="opacity: 0.6; font-size: 0.9rem; margin-bottom: 20px;">Vincula tu cuenta de Mobile Legends para que podamos validar tu nivel competitivo.</p>
                <button class="btn btn-outline btn-sm" onclick="alert('Consulta con soporte para vincular un nuevo ID')">Contactar Soporte</button>
            </div>
        `;
        ...
        if (commWall) {
            commWall.innerHTML = `
                <div class="comm-grid" style="display: flex; flex-direction: column; gap: 30px;">
                    <!-- Contenido Destacado / Videos -->
                    <div class="comm-videos glass" style="padding: 40px; border-radius: 20px;">
                        <h3 style="color: var(--accent-orange); margin-bottom: 30px; text-align: center; text-transform: uppercase;">Contenido Destacado</h3>
                        
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 40px; align-items: start;">
                            <div class="video-card">
                                <div style="resize: both; overflow: hidden; max-width: 100%; border: 3px solid #800080; box-shadow: 0 0 15px #FF8C00; border-radius: 12px; background: #000; margin-bottom: 20px;">
                                    <video controls preload="metadata" playsinline muted style="width: 100%; height: 100%; display: block; object-fit: contain;" src="video/PRESENTACION%20AMANECER.mp4#t=6" onplay="if(!this.playedOnce){this.currentTime=0;this.playedOnce=true;}"></video>
                                </div>
                                <h4 style="color: #fff; margin-bottom: 10px; font-size: 1.2rem;">PRESENTACIÓN AMANECER</h4>
                                <p style="font-size: 0.9rem; line-height: 1.5; color: var(--text-secondary); margin-bottom: 15px; text-align: justify; opacity: 0.9;">
                                    ¡Sean bienvenidos a este nuevo Amanecer! Nos renovamos y traemos muchas cosas nuevas, todo lo que necesitas para el competitivo lo vas a encontrar en un solo lugar, ahora es tu decisión, ¿estás listo para superar tus límites? Regístrate y activa todos los beneficios que Amanecer tiene para vos.
                                </p>
                                <p style="font-size: 0.7rem; color: var(--accent-orange); margin-top: 8px; font-style: italic;"><i class="fas fa-arrows-alt"></i> Arrastra la esquina inferior derecha del recuadro para ajustar el tamaño del video.</p>
                            </div>

                            <div class="video-card">
                                <div style="resize: both; overflow: hidden; max-width: 100%; border: 3px solid #800080; box-shadow: 0 0 15px #FF8C00; border-radius: 12px; background: #000; margin-bottom: 20px;">
                                    <video controls preload="metadata" playsinline muted style="width: 100%; height: 100%; display: block; object-fit: contain;" src="video/CIRCULO%20CLASIFICATORIO%20COMPLETO.mp4#t=5.5" onplay="if(!this.playedOnce){this.currentTime=0;this.playedOnce=true;}"></video>
                                </div>
                                <h4 style="color: #fff; margin-bottom: 10px; font-size: 1.2rem;">CIRCUITO CLASIFICATORIO</h4>
                                <p style="font-size: 0.9rem; line-height: 1.5; color: var(--text-secondary); margin-bottom: 15px; text-align: justify; opacity: 0.9;">
                                    ¿Estás preparado para el circuito clasificatorio? Solo los más fuertes clasifican, solo las leyendas triunfan. El sistema de puntos de nuestra autoría que viene a revolucionar la tierra del amanecer obligandonos a superar nuestros límites, la dedicación y la constancia se harán presentes en este circuito, ¿se van a superar o se van a estancar?
                                </p>
                                <p style="font-size: 0.7rem; color: var(--accent-orange); margin-top: 8px; font-style: italic;"><i class="fas fa-arrows-alt"></i> Arrastra la esquina inferior derecha del recuadro para ajustar el tamaño del video.</p>
                            </div>
                        </div>
    ...
    function renderAbout() {
        ...
        const staffCards = APP_DATA.team.map(member => `
            <div class="staff-card glass" style="text-align: center; padding: 20px; border-radius: 20px; border: 1px solid rgba(255,165,0,0.1); transition: 0.3s; background: rgba(0,0,0,0.2);">
                <div class="staff-photo-wrapper" style="width: 100px; height: 100px; margin: 0 auto 15px; position: relative;">
                    <img src="${encodeURI(member.photo)}?v=${new Date().getTime()}" 
                         alt="${member.name}" 
                         style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%; border: 2px solid var(--accent-orange); box-shadow: 0 0 15px rgba(255,165,0,0.1);"
                ...
                <h4 style="color: #fff; font-size: 1.1rem; margin-bottom: 5px; font-weight: 900; letter-spacing: 1px;">${member.name}</h4>
                <p style="color: var(--accent-orange); font-size: 0.75rem; font-weight: bold; text-transform: uppercase; margin-bottom: 12px;">${member.role}</p>
                <p style="font-size: 0.75rem; line-height: 1.5; opacity: 0.7; font-style: italic; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 10px;">
                    "${member.bio || ''}"
                </p>
            </div>
        `).join('');
        ...
        container.innerHTML = `
            <div class="about-layout" style="display: flex; flex-direction: column; gap: 50px; animation: fadeIn 0.6s ease;">
                <div class="mission-section glass" style="padding: clamp(30px, 5vw, 60px); border-radius: 30px; background: linear-gradient(135deg, rgba(255,255,255,0.02), rgba(255,165,0,0.02)); border: 1px solid rgba(255,165,0,0.1);">
                    <div style="display: flex; align-items: center; gap: 20px; margin-bottom: 25px;">
                        ...
                        <h2 style="font-size: clamp(1.8rem, 4vw, 2.5rem); color: #fff;">${t('mission_vision')}</h2>
                    </div>
                    <p style="line-height: 1.8; font-size: 1.1rem; opacity: 0.9; max-width: 900px;">
                        ${APP_DATA.organization.mission}
                    </p>
                    <div class="values-grid mt-30" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px;">
                        ${APP_DATA.organization.values.map(v => `
                            <div style="background: rgba(255,255,255,0.03); padding: 15px; border-radius: 12px; font-size: 0.9rem; border-left: 3px solid var(--accent-orange); font-weight: bold;">
                                <i class="fas fa-check-circle" style="color: var(--accent-orange); margin-right: 10px;"></i> ${v}
                            </div>
                        `).join('')}
                    </div>
                </div>
                ...
                <div class="staff-section">
                    <h3 style="color: #fff; font-size: 2rem; margin-bottom: 40px; text-align: center; text-transform: uppercase; letter-spacing: 5px;">
                        <span style="color: var(--accent-orange);">NUESTRO</span> STAFF EXPERTO
                    </h3>
                    <div class="staff-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 30px;">
                        ${staffCards}
                    </div>
                </div>
            </div>
                ...
                <div class="glass" style="padding: clamp(20px, 5vw, 40px); border-radius: 20px; position: relative; overflow: hidden; background: linear-gradient(135deg, rgba(45,0,85,0.6), rgba(26,0,51,0.8)); margin-bottom: 40px;">
                    <div class="squad-header" style="display: flex; align-items: center; gap: 30px; margin-bottom: 30px; flex-wrap: wrap;">
                        <div class="logo-wrapper">
                            <img src="${encodeURI(squad.logo)}" alt="Logo ${squad.title}" style="width: 150px; height: 150px; border-radius: 15px;" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
    ...
    function renderTeamView(roster) {
        function renderPlayerSquare(player) {
            ...
            const photoHtml = player.photo ? 
                `<img src="${encodeURI(player.photo)}?v=${new Date().getTime()}" 
                      alt="${player.name}" 
            ...
            return `
                <div class="player-square glass" style="display: flex; flex-direction: column; align-items: center; padding: 25px 15px; text-align: center; border-radius: 20px; border: 1px solid rgba(255,140,0,0.1); position:relative; overflow:hidden; gap: 10px; min-height: 400px; justify-content: flex-start;">
                    <div class="player-avatar-placeholder" style="width: 50%; aspect-ratio: 1/1; background: rgba(255,140,0,0.05); border: 2px dashed rgba(255,140,0,0.2); border-radius: 8px; margin-bottom: 5px; display: flex; align-items: center; justify-content: center; color: var(--accent-orange); font-size: 1.5rem; overflow: hidden;">
                        ${photoHtml}
                    </div>
                    <div style="font-weight: 900; color: #fff; font-size: 0.8rem; text-transform: uppercase;">${player.name}</div>
                    <div style="font-size: 0.6rem; color: var(--accent-orange); font-weight: bold;">${player.role}</div>
                    <div style="font-size: 0.55rem; opacity: 0.5; margin-top: 5px;">${player.specialty || ''}</div>
                    ${player.bio ? `<div style="font-size: 0.6rem; line-height: 1.4; opacity: 0.8; font-style: italic; margin-top: 10px; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 10px;">"${player.bio}"</div>` : ''}
                    
                    ${hasHighlight ? `
                        <button class="highlight-btn" onclick="toggleHighlight('video-${player.name.replace(/\\s+/g, '')}')">
                            <i class="fas fa-play-circle"></i> ${t('best_play')}
                        </button>
                        <div id="video-${player.name.replace(/\\s+/g, '')}" class="player-highlight-container hidden-video">
        ...
        return `
            <div class="team-view" style="animation: fadeInUp 0.5s ease;">
                ...
                <div class="roster-grid" style="display: grid; grid-template-columns: repeat(min(5, max(1, ${roster.starters.length})), 1fr); gap: 10px; margin-bottom: 30px;">
