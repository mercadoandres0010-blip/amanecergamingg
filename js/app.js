...
// Global rule: ensure only one video plays at a time
    ...
    if (APP_DATA) {
        ...
        if (hasOldPaths || hasOldTournamentData || hasOldTeamData || hasOldSquadsData || hasOldBranding || hasAllStarsOctavos) {
            APP_DATA = INITIAL_DATA;
            localStorage.setItem('amanecer_data', JSON.stringify(APP_DATA));
            localStorage.setItem('amanecer_data_v', '4.0'); // Bump version to 4.0
        }
    }
